[使用说明.md](https://github.com/user-attachments/files/29826499/default.md)
# OCPP 1.6 CSMS 使用说明
...
| `<电脑本地以太网IP>:9000/{id}`      | 充电桩连接地址         |

## 充电桩连接

充电桩端配置 OCPP 端点地址为 `ws://{服务器IP}:9000/{充电桩ID}`。桩网关和本地ip正常按照以太网联PC方式设置。例如本机测试：

```
ws://localhost:9000/CP_001
<电脑本地以太网IP>:9000/{id} #上面的不行就尝试用下面这个域名
```
