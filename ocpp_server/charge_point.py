...
class ChargePointHandler(BaseChargePoint):
    ...
    def __init__(self, charge_box_id: str, connection):
        ...
        self._charge_point_db_id: int | None = None  # 数据库中的 charge_point.id
        self._pending_actions: dict[str, str] = {}  # msg_id → action 映射
    ...
    async def route_message(self, raw_msg):
        ...
        # CALLRESULT（type 3）或 CALLERROR（type 4）：从 _pending_actions 取 action 名
        if msg_type in (3, 4):
    ...
    async def _send(self, message):
        ...
        # CALL 消息：记录 msg_id → action 映射，供 CALLRESULT 使用
        if msg_type == 2 and action:
            self._pending_actions[msg_id] = action
        # CALLRESULT/CALLERROR：从映射中取回对应的 action 名称
    ...
    async def on_boot_notification(
        ...
        """充电桩启动注册"""
        logger.info(
            f"[{self.id}] BootNotification: vendor={charge_point_vendor}, "
            f"model={charge_point_model}, kwargs={kwargs}"
        )
        ...
        status = RegistrationStatus.accepted if not cfg else (
            RegistrationStatus.accepted if cfg.status == "Accepted" else
            RegistrationStatus.pending if cfg.status == "Pending" else
            RegistrationStatus.rejected
        )
        interval = 60 if not cfg else getattr(cfg, 'interval', 60)

        if cfg and cfg.behavior == "Delay" and cfg.delay > 0:
            await asyncio.sleep(cfg.delay)
        if cfg and cfg.behavior == "Drop":
            return None
    ...
    async def on_authorize(self, id_tag: str):
        ...
        if cfg:
            ...
            if cfg.status != "Accepted":
                logger.info(f"[{self.id}] Authorize overridden by handler config: {cfg.status}")
    ...
    async def on_start_transaction(
        ...
        """开始充电事务"""
        logger.info(
            f"[{self.id}] StartTransaction: connector={connector_id}, "
        ...
        async with async_session() as db:
            ...
            tx_id = await transaction_service.start_transaction(
                db=db,
                charge_point_id=self._charge_point_db_id or 0,
                connector_id=connector_id,
                id_tag=id_tag,
                meter_start=meter_start,
                start_timestamp=datetime.fromisoformat(timestamp),
            )
            await cp_service.update_connector_status(
                db, self._charge_point_db_id or 0, connector_id, "Charging"
            )
            await db.commit()
    ...
    async def on_stop_transaction(
        self, transaction_id: int, timestamp: str,
        ...
        async with async_session() as db:
            await transaction_service.stop_transaction(
                db=db,
            ...
            if tx and self._charge_point_db_id:
                await cp_service.update_connector_status(
                    db, self._charge_point_db_id, tx.connector_id, "Available"
                )
            await db.commit()
    ...
    async def on_meter_values(self, connector_id: int, meter_value: list, **kwargs):
        ...
        logger.debug(
            f"[{self.id}] MeterValues: connector={connector_id}, "
            f"tx={transaction_id}, samples={len(meter_value)}"
        )
        publish(OcppMessage(charge_box_id=self.id, direction="IN", action="MeterValues (CALL)",
                            payload={"connector_id": connector_id, "transaction_id": transaction_id,
                                      "samples": len(meter_value),
                                      "values": [
                                          {sv.get("measurand", "?"): sv.get("value", "?")
                                           for sv in s.get("sampledValue", [])}
                                          for s in meter_value
                                      ]}))
        async with async_session() as db:
            for sample in meter_value:
                await transaction_service.add_meter_value(
                    db=db,
    ...
    async def on_status_notification(
        ...
        async with async_session() as db:
            ...
            if self._charge_point_db_id:
                await cp_service.update_connector_status(
                    db, self._charge_point_db_id, connector_id, status, error_code
                )
            await db.commit()
    ...
    async def on_diagnostics_status_notification(self, status: str):
        ...
        publish(OcppMessage(charge_box_id=self.id, direction="IN", action="DiagnosticsStatusNotification (CALL)",
    ...
    async def on_firmware_status_notification(self, status: str):
        ...
        publish(OcppMessage(charge_box_id=self.id, direction="IN", action="FirmwareStatusNotification (CALL)",
                            payload={"status": status}))
    ...
    async def send_reset(self, reset_type: str = "Soft") -> str:
        """向充电桩下发 Reset 命令，返回状态"""
        from ocpp.v16.enums import ResetType as RT
        req_type = RT.soft if reset_type.lower() == "soft" else RT.hard
        resp: call_result.Reset = await self.call(
            call.Reset(type=req_type)
        )
        return resp.status

    async def send_remote_start(
        self, id_tag: str, connector_id: int = 1
    ) -> str:
        """远程启动充电"""
        resp: call_result.RemoteStartTransaction = await self.call(
    ...
    async def send_remote_stop(self, transaction_id: int) -> str:
        """远程停止充电"""
        resp: call_result.RemoteStopTransaction = await self.call(
            call.RemoteStopTransaction(transaction_id=transaction_id)
        )
        return resp.status

    async def send_change_configuration(
        self, key: str, value: str
    ) -> str:
        """修改配置"""
        resp: call_result.ChangeConfiguration = await self.call(
    ...
    async def send_unlock_connector(self, connector_id: int) -> str:
        """解锁连接器"""
        resp: call_result.UnlockConnector = await self.call(
    ...
    async def send_trigger_message(self, requested_message: str, connector_id: int = None):
        ...
        try:
            ...
            logger.warning(f"[{self.id}] TriggerMessage timeout (CP not responding)")
            raise
    ...
    async def send_update_firmware(
        ...
        """下发固件更新命令（UpdateFirmware.conf 为空响应，无 status 字段）"""
        await self.call(
            call.UpdateFirmware(
                location=location,
    ...
    async def send_set_charging_profile(
        ...
        """设置充电曲线"""
        resp: call_result.SetChargingProfile = await self.call(
    ...
    async def send_clear_charging_profile(
        ...
        if charging_profile_purpose is not None:
            purpose_map = {
                "ChargePointMaxProfile": ChargingProfilePurposeType.charge_point_max_profile,
                "TxDefaultProfile": ChargingProfilePurposeType.tx_default_profile,
                "TxProfile": ChargingProfilePurposeType.tx_profile,
            }
            kwargs["charging_profile_purpose"] = purpose_map.get(
                charging_profile_purpose, ChargingProfilePurposeType.tx_default_profile
            )
        if stack_level is not None:
            kwargs["stack_level"] = stack_level
        resp: call_result.ClearChargingProfile = await self.call(
    ...
    async def send_send_local_list(
        self, list_version: int, update_type: str,
    ...
    async def send_cancel_reservation(self, reservation_id: int) -> str:
        """取消预约"""
        resp: call_result.CancelReservation = await self.call(
            call.CancelReservation(reservation_id=reservation_id)
        )
        return resp.status

    async def send_reserve_now(
        self, connector_id: int, expiry_date: str,
        id_tag: str, reservation_id: int = None,
        ...
        resp: call_result.ReserveNow = await self.call(
            call.ReserveNow(**kwargs)
        )
        return resp.status
